...
async def main():
    ...
    try:
        if not await asyncio.to_thread(
            session.login_session_file, Path(args.session_file)
        ):
            logger.error("Failed to log in using session file.")
            sys.exit(1)
    except tidalapi.exceptions.AuthenticationError:
        logger.error(
            "Authentication failed. Your session may have expired or the session file is invalid. "
            "You might need to create a new session file."
        )
        sys.exit(1)
